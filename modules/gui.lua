pfUI:RegisterModule("gui", "vanilla:tbc", function()
  ...
  do -- DropDown Menus
    ...
    for i = 0, getn(_G.ITEM_QUALITY_COLORS) - 2 do
      local entry = string.format("%d:%s", i,
        string.format("%s%s%s", _G.ITEM_QUALITY_COLORS[i].hex, _G[string.format("ITEM_QUALITY%d_DESC", i)],
  ...
  do -- Generate Config UI
    ...
    CreateGUIEntry(T["Settings"], T["General"], function()
      ...
      CreateConfig(nil, T["Save profile"], C.global, "profile", "button", function()
        if C.global.profile and pfUI_profiles[C.global.profile] then
          CreateQuestionDialog(T["Save current settings to profile"] .. " '|cff33ffcc" .. C.global.profile .. "|r'?",
            function()
      ...
      CreateConfig(U["pixelperfect"], T["Enable UI-Scale"], C.global, "pixelperfect", "dropdown",
      ...
      CreateConfig(nil, T["Threshold To Trust Health Estimation"], C.global, "libhealth_hit", "dropdown",
        pfUI.gui.dropdowns.uf_rangecheckinterval)
      CreateConfig(nil, T["Required Damage In Percent"], C.global, "libhealth_dmg", "dropdown",
        pfUI.gui.dropdowns.percent_small)
    ...
    CreateGUIEntry(T["Settings"], T["Appearance"], function()
      ...
      CreateConfig(U["mapcolors"], T["Map Group/Raid Circle Size"], C.appearance.worldmap, "groupcircles", "dropdown",
        pfUI.gui.dropdowns.mapcircle)
      CreateConfig(U["mapcolors"], T["Colorize player name on WorldMap and BattlefieldMinimap"], C.appearance.worldmap,
        "colornames", "checkbox")
      CreateConfig(U["map"], T["Map Tooltip Scale"], C.appearance.worldmap, "tooltipsize", "dropdown",
        pfUI.gui.dropdowns.maptooltip)
      CreateConfig(nil) -- spacer
      CreateConfig(nil, T["Enable Frame Shadow"], C.appearance.border, "shadow", "checkbox")
      CreateConfig(nil, T["Frame Shadow Intensity"], C.appearance.border, "shadow_intensity", "dropdown",
        pfUI.gui.dropdowns.uf_debuff_indicator_size)
      CreateConfig(nil) -- spacer
      CreateConfig(nil, T["Force Blizzard Borders (|cffffaaaaExperimental|r)"], C.appearance.border, "force_blizz",
        "checkbox")
      ...
      CreateConfig(nil, T["Action Bar Border Size"], C.appearance.border, "actionbars", "dropdown",
        pfUI.gui.dropdowns.border)
      CreateConfig(nil, T["Unit Frame Border Size"], C.appearance.border, "unitframes", "dropdown",
        pfUI.gui.dropdowns.border)
      ...
      CreateConfig(U["nameplates"], T["Nameplate Border Size"], C.appearance.border, "nameplates", "dropdown",
        pfUI.gui.dropdowns.border)
      CreateConfig(nil) -- spacer
      CreateConfig(U["infight"], T["Enable Combat Glow Effects On Screen Edges"], C.appearance.infight, "screen",
        "checkbox")
      CreateConfig(U["infight"], T["Enable Aggro Glow Effects On Screen Edges"], C.appearance.infight, "aggro",
        "checkbox")
      CreateConfig(U["infight"], T["Enable Low Health Glow Effects On Screen Edges"], C.appearance.infight, "health",
        "checkbox")
      CreateConfig(U["infight"], T["Screen Edge Glow Intensity"], C.appearance.infight, "intensity", "dropdown",
        pfUI.gui.dropdowns.glowintensity)
    end)
    ...
    CreateGUIEntry(T["Settings"], T["Screenshot"], function()
      ...
      CreateConfig(nil, T["Battleground Statistics"], C.screenshot, "battleground", "dropdown",
        pfUI.gui.dropdowns.screenshot_battle)
    ...
    CreateGUIEntry(T["Unit Frames"], T["General"], function()
      ...
      CreateConfig(nil, T["Healthbar Animation Speed"], C.unitframes, "animation_speed", "dropdown",
      ...
      CreateConfig(nil, T["Unit Frame Text Style"], C.global, "font_unit_style", "dropdown", pfUI.gui.dropdowns
        .fontstyle)
      ...
      CreateConfig(nil, T["Enable 40y-Range Check"], C.unitframes, "rangecheck", "checkbox", nil, nil, nil, nil,
        "vanilla")
      CreateConfig(nil, T["Range Check Interval"], C.unitframes, "rangechecki", "dropdown",
        pfUI.gui.dropdowns.uf_rangecheckinterval, nil, nil, nil, "vanilla")
      ...
      CreateConfig(nil, T["Druid Mana Bar Text"], C.unitframes, "druidmanatext", "checkbox", nil, nil, nil, nil,
        "vanilla")
    end)
    ...
    CreateGUIEntry(T["Unit Frames"], T["Click Casting"], function()
      for id, data in ipairs(unitframeSettings) do
        CreateConfig(U[data[1]], T["Enable"] .. " " .. data[2] .. " " .. T["Click Casting"], C.unitframes[data[1]],
          "clickcast", "checkbox")
      end
    ...
    for id, data in ipairs(unitframeSettings) do
      ...
      CreateGUIEntry(T["Unit Frames"], t, function()
        ...
        CreateConfig(U[c], T["Default Transparency"], C.unitframes[c], "alpha_visible", "dropdown",
          pfUI.gui.dropdowns.percent_small)
        CreateConfig(U[c], T["Out Of Range Transparency"], C.unitframes[c], "alpha_outrange", "dropdown",
          pfUI.gui.dropdowns.percent_small)
        CreateConfig(U[c], T["Offline Transparency"], C.unitframes[c], "alpha_offline", "dropdown",
          pfUI.gui.dropdowns.percent_small)
        ...
        CreateConfig(U[c], T["Portrait Position"], C.unitframes[c], "portrait", "dropdown",
        ...
        CreateConfig(U[c], T["Health Bar Texture"], C.unitframes[c], "bartexture", "dropdown",
          pfUI.gui.dropdowns.uf_bartexture)
        CreateConfig(U[c], T["Power Bar Texture"], C.unitframes[c], "pbartexture", "dropdown",
          pfUI.gui.dropdowns.uf_bartexture)
        ...
        CreateConfig(U[c], T["PvP Flag Position"], C.unitframes[c], "pvpiconalign", "dropdown",
        ...
        if c == "pet" then
          CreateConfig(U[c], T["Show Happiness Icon"], C.unitframes[c], "happinessicon", "dropdown",
            pfUI.gui.dropdowns.uf_happiness)
        ...
        CreateConfig(U[c], T["Raid Mark Position"], C.unitframes[c], "raidiconalign", "dropdown",
          pfUI.gui.dropdowns.positions)
        ...
        CreateConfig(U[c], T["Display Overheal"], C.unitframes[c], "overhealperc", "dropdown",
          pfUI.gui.dropdowns.uf_overheal)
        ...
        if c == "raid" then
          ...
          CreateConfig(U["raid"], T["Raid Layout"], C.unitframes[c], "raidlayout", "dropdown",
            pfUI.gui.dropdowns.uf_raidlayout)
          CreateConfig(U["raid"], T["Raid Fill Direction"], C.unitframes[c], "raidfill", "dropdown",
        ...
        CreateConfig(U[c], T["Power Bar Anchor"], C.unitframes[c], "panchor", "dropdown",
        ...
        CreateConfig(U[c], T["Combat Text Font"], C.unitframes[c], "hitindicatorfont", "dropdown",
        ...
        CreateConfig(U[c], T["Debuff Position"], C.unitframes[c], "debuffs", "dropdown",
        ...
        if c ~= "player" then
          CreateConfig(U[c], T["Only Show Own Debuffs (|cffffaaaaExperimental|r)"], C.unitframes[c], "selfdebuff",
            "checkbox")
        end
        ...
        CreateConfig(U[c], T["Indicator Position"], C.unitframes[c], "squarepos", "dropdown",
          pfUI.gui.dropdowns.positions)
        ...
        CreateConfig(U[c], T["Indicator Spacing"], C.unitframes[c], "indicator_spacing", "dropdown",
          pfUI.gui.dropdowns.spacing)
        CreateConfig(U[c], T["Indicator Position"], C.unitframes[c], "indicator_pos", "dropdown",
          pfUI.gui.dropdowns.positions)
        ...
        CreateConfig(U[c], T["Show Dispel Indicators"], C.unitframes[c], "debuff_indicator", "dropdown",
          pfUI.gui.dropdowns.uf_debuff_indicator)
        CreateConfig(U[c], T["Only Class Dispellable"], C.unitframes[c], "debuff_ind_class", "checkbox")
        CreateConfig(U[c], T["Indicator Position"], C.unitframes[c], "debuff_ind_pos", "dropdown",
          pfUI.gui.dropdowns.positions)
        CreateConfig(U[c], T["Indicator Size"], C.unitframes[c], "debuff_ind_size", "dropdown",
          pfUI.gui.dropdowns.uf_debuff_indicator_size)
        ...
        CreateConfig(U[c], T["Custom Font Name"], C.unitframes[c], "customfont_name", "dropdown",
          pfUI.gui.dropdowns.fonts)
        CreateConfig(U[c], T["Custom Font Size"], C.unitframes[c], "customfont_size")
        CreateConfig(U[c], T["Custom Font Style"], C.unitframes[c], "customfont_style", "dropdown",
          pfUI.gui.dropdowns.fontstyle)
    ...
    CreateGUIEntry(T["Bags & Bank"], nil, function()
      CreateConfig(nil, T["Disable Item Quality Color For \"Common\" Items"], C.appearance.bags, "borderlimit",
        "checkbox")
      CreateConfig(nil, T["Enable Item Quality Color For Equipment Only"], C.appearance.bags, "borderonlygear",
        "checkbox")
    ...
    CreateGUIEntry(T["Loot"], nil, function()
      ...
      CreateConfig(nil, T["Random Roll Announcement Rarity"], C.loot, "rollannouncequal", "dropdown",
        pfUI.gui.dropdowns.loot_rarity)
    ...
    CreateGUIEntry(T["Minimap"], T["Minimap"], function()
      ...
      CreateConfig(nil, T["Zone Text On Minimap"], C.appearance.minimap, "zonetext", "dropdown",
        pfUI.gui.dropdowns.minimap_zone_visibility)
      CreateConfig(nil, T["Coordinates On Minimap"], C.appearance.minimap, "coordstext", "dropdown",
        pfUI.gui.dropdowns.minimap_cords_visibility)
      CreateConfig(nil, T["Coordinates Location"], C.appearance.minimap, "coordsloc", "dropdown",
    ...
    CreateGUIEntry(T["Minimap"], T["Addon Buttons"], function()
      ...
      CreateConfig(U["addonbuttons"], T["Addon Buttons Panel Position"], C.abuttons, "position", "dropdown",
    ...
    CreateGUIEntry(T["Buffs"], T["Target Debuff Bar"], function()
      ...
      CreateConfig(nil, T["Only Show Own Debuffs (|cffffaaaaExperimental|r)"], C.buffbar.tdebuff, "selfdebuff",
        "checkbox")
    ...
    CreateGUIEntry(T["Actionbar"], T["General"], function()
      ...
      CreateConfig(U["bars"], T["Button Animation"], C.bars, "animation", "dropdown",
        pfUI.gui.dropdowns.actionbuttonanimations)
      CreateConfig(U["bars"], T["Button Animation Trigger"], C.bars, "animmode", "dropdown",
        pfUI.gui.dropdowns.animationmode)
    ...
    CreateGUIEntry(T["Actionbar"], T["Gryphons"], function()
      ...
      CreateConfig(U["gryphons"], T["Left Anchor"], C.bars.gryphons, "anchor_left", "dropdown",
        pfUI.gui.dropdowns.xpanchors)
      CreateConfig(U["gryphons"], T["Right Anchor"], C.bars.gryphons, "anchor_right", "dropdown",
        pfUI.gui.dropdowns.xpanchors)
    ...
    for _, data in pairs(barnames) do
      ...
      local formfactors = function()
        return BarLayoutOptions(tonumber(C.bars["bar" .. id].buttons) or id < 11 and NUM_ACTIONBAR_BUTTONS or
          id > 11 and NUM_SHAPESHIFT_SLOTS or NUM_PET_ACTION_SLOTS)
      end
      ...
      CreateGUIEntry(T["Actionbar"], caption, function()
        ...
        if id ~= 11 and id ~= 12 then
          CreateConfig(U["bars"], T["Buttons"], C.bars["bar" .. id], "buttons", "dropdown",
        ...
        CreateConfig(U["bars"], T["Spacing"], C.bars["bar" .. id], "spacing", "dropdown",
          pfUI.gui.dropdowns.actionbarbuttons)
    ...
    CreateGUIEntry(T["XP Bar"], T["Experience Bar"], function()
      ...
      CreateConfig(U["xpbar"], T["Aligned Position"], C.panel.xp, "xp_position", "dropdown",
        pfUI.gui.dropdowns.xp_position)
    ...
    CreateGUIEntry(T["XP Bar"], T["Reputation Bar"], function()
      ...
      CreateConfig(U["xpbar"], T["Aligned Position"], C.panel.xp, "rep_position", "dropdown",
        pfUI.gui.dropdowns.xp_position)
    ...
    CreateGUIEntry(T["Tooltip"], nil, function()
      ...
      CreateConfig(nil, T["Cursor Tooltip Align"], C.tooltip, "cursoralign", "dropdown", pfUI.gui.dropdowns
        .tooltip_align)
      ...
      CreateConfig(nil, T["Status Bar Texture"], C.tooltip.statusbar, "texture", "dropdown",
        pfUI.gui.dropdowns.uf_bartexture)
    ...
    CreateGUIEntry(T["Castbar"], nil, function()
      ...
      CreateConfig(nil, T["Castbar Texture"], C.appearance.castbar, "texture", "dropdown",
        pfUI.gui.dropdowns.uf_bartexture)
    ...
    CreateGUIEntry(T["Chat"], nil, function()
      ...
      CreateConfig(nil, T["Who Search Unknown Classes (|cffffaaaaExperimental|r)"], C.chat.text, "whosearchunknown",
        "checkbox")
    ...
    CreateGUIEntry(T["Nameplates"], nil, function()
      ...
      CreateConfig(U["nameplates"], T["Vertical Offset (|cffffaaaaExperimental|r)"], C.nameplates, "vertical_offset", nil,
        nil, nil, nil, nil, "vanilla")
      CreateConfig(U["nameplates"], T["Inactive Nameplate Alpha"], C.nameplates, "notargalpha", "dropdown",
        pfUI.gui.dropdowns.percent_small)
      ...
      CreateConfig(U["nameplates"], T["Target Nameplate Zoom Factor"], C.nameplates, "targetzoomval", "dropdown",
        pfUI.gui.dropdowns.percent_small)
      ...
      CreateConfig(U["nameplates"], T["Enable Class Colors On Friends Name"], C.nameplates, "friendclassnamec",
        "checkbox")
      CreateConfig(U["nameplates"], T["Enable Combo Point Display"], C.nameplates, "cpdisplay", "checkbox")
      CreateConfig(U["nameplates"], T["Enable Clickthrough"], C.nameplates, "clickthrough", "checkbox", nil, nil, nil,
        nil, "vanilla")
      CreateConfig(U["nameplates"], T["Enable Overlap"], C.nameplates, "overlap", "checkbox", nil, nil, nil, nil,
        "vanilla")
      CreateConfig(U["nameplates"], T["Enable Mouselook With Right Click"], C.nameplates, "rightclick", "checkbox", nil,
        nil, nil, nil, "vanilla")
      CreateConfig(U["nameplates"], T["Right Click Auto Attack Threshold"], C.nameplates, "clickthreshold", nil, nil, nil,
        nil, nil, "vanilla")
      CreateConfig(U["nameplates"], T["Use Unit Fonts"], C.nameplates, "use_unitfonts", "checkbox")
      CreateConfig(U["nameplates"], T["Font Style"], C.nameplates.name, "fontstyle", "dropdown",
        pfUI.gui.dropdowns.fontstyle)
      ...
      CreateConfig(U["nameplates"], T["Raid Icon Position"], C.nameplates, "raidiconpos", "dropdown",
        pfUI.gui.dropdowns.positions)
      ...
      CreateConfig(U["nameplates"], T["Debuff Position"], C.nameplates.debuffs, "position", "dropdown",
        pfUI.gui.dropdowns.debuffposition)
      ...
      CreateConfig(U["nameplates"], T["Only Show Own Debuffs (|cffffaaaaExperimental|r)"], C.nameplates, "selfdebuff",
        "checkbox")
      CreateConfig(U["nameplates"], T["Filter Mode"], C.nameplates.debuffs, "filter", "dropdown",
      ...
      CreateConfig(U["nameplates"], T["Healthbar Texture"], C.nameplates, "healthtexture", "dropdown",
        pfUI.gui.dropdowns.uf_bartexture)
      CreateConfig(U["nameplates"], T["Show Health Points"], C.nameplates, "showhp", "checkbox")
      CreateConfig(U["nameplates"], T["Health Text Position"], C.nameplates, "hptextpos", "dropdown",
        pfUI.gui.dropdowns.textalign)
      CreateConfig(U["nameplates"], T["Health Text Format"], C.nameplates, "hptextformat", "dropdown",
        pfUI.gui.dropdowns.hpformat)
      ...
      CreateConfig(nil, T["SuperWoW Settings"], nil, nil, "header")
      CreateConfig(U["nameplates"], T["Overwrite Border Color With Combat State"], C.nameplates, "outcombatstate",
        "checkbox")
      CreateConfig(U["nameplates"], T["Overwrite Health Color With Combat State"], C.nameplates, "barcombatstate",
        "checkbox")
      CreateConfig(U["nameplates"], T["Overwrite If Unit Is Attacking You"], C.nameplates, "ccombatthreat", "checkbox")
      CreateConfig(U["nameplates"], T["Overwrite If Unit Is Attacking Off-Tank"], C.nameplates, "ccombatofftank",
        "checkbox")
      CreateConfig(U["nameplates"], T["Overwrite If Unit Is Attacking Others"], C.nameplates, "ccombatnothreat",
        "checkbox")
    ...
    CreateGUIEntry(T["Thirdparty"], T["Integrations"], function()
      ...
      CreateConfig(nil, "Recount (" .. T["Skin"] .. ")", C.thirdparty.recount, "skin", "checkbox", nil, nil, nil, nil,
        "tbc")
      CreateConfig(nil, "Recount (" .. T["Dock"] .. ")", C.thirdparty.recount, "dock", "checkbox", nil, nil, nil, nil,
        "tbc")
      ...
      CreateConfig(nil, "SWStats (" .. T["Skin"] .. ")", C.thirdparty.swstats, "skin", "checkbox", nil, nil, nil, nil,
        "vanilla")
      CreateConfig(nil, "SWStats (" .. T["Dock"] .. ")", C.thirdparty.swstats, "dock", "checkbox", nil, nil, nil, nil,
        "vanilla")
      CreateConfig(nil, "KLH Threat Meter (" .. T["Skin"] .. ")", C.thirdparty.ktm, "skin", "checkbox", nil, nil, nil,
        nil, "vanilla")
      CreateConfig(nil, "KLH Threat Meter (" .. T["Dock"] .. ")", C.thirdparty.ktm, "dock", "checkbox", nil, nil, nil,
        nil, "vanilla")
